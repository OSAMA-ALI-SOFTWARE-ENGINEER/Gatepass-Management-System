...
$ok = mysqli_query($con, "UPDATE vehicles SET ExitDate = NOW() WHERE id=$id");
$msg = $ok ? 'Exit time recorded.' : 'Unable to record exit time.';
...
if ($return !== '' && strpos($return, '://') === false) {
  // Only allow relative redirects within app, and only to managevehicle.php
